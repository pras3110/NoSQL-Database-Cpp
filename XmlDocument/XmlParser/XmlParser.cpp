...
void XmlParser::processText(ElemStack& elemStack)
{
  ...
  if (verbose_) std::cout << "\n";
  std::cout << "checking first element" << elemStack.top();
}
...
int main()
{
  ...
  //putline();
  std::string src = "../XmlElementParts/db.xml";
  ...
  for (auto x : vec) {
	  ...
	  std::cout<<x->children()[0]->children()[0]->value() <<std::endl;
			  ...
			  for (auto z : y->children()) {
				  ...
				  if (z->value()=="category") {
					  ...
					  for (auto a : z->children()) {
						  std::cout << a->children()[0]->value();
					  }
				  }
				  else {
					  std::cout << z->children()[0]->value();
				  }
			  }
 ...
 // std::cout<<vec[0]->children()[1]->children()[0]->children()[0]->value()     <<" PRasanna1"<<std::endl;

  

  //std::vector<sptr> desc = pDoc->descendents("reference").select();
  //std::cout << vec.size();
  //std::cout << desc[0]->children()[1]->value() << " PRasanna" << std::endl;
